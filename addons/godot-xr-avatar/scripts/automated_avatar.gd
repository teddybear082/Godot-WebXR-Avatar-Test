...
func _ready():
	...
	#height offset is the distance between player's eye height and assumed top of head
	#average head is 1/8th of person's height, and eyes on average are half of player's head
	#So remaining distance from eyes to top of head should be roughly 1/16 of player's height
	height_offset = ((16.0*get_current_player_height())/15.0)-get_current_player_height()
	PlayerMasterControls.player_height_offset = height_offset
	$Armature.scale *= (get_current_player_height()+height_offset)/avatar_height
	armature_scale = $Armature.scale
	max_height = get_current_player_height() # +height_offset
...
func _physics_process(delta: float) -> void:
	if PlayerMasterControls.player_height_offset != last_height_offset:
		player_body.player_height_offset = PlayerMasterControls.player_height_offset
		last_height_offset = player_body.player_height_offset
